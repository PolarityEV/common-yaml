...
#define VCU_I2C_CHARGER_CONTROL        (0x0C)  ///< Charger control (WRITE-ONLY: TX1: 4 bytes total [command + 3 data: enable, voltage%, current]; TX2: None - I2C ACK confirms) [v8.0: two-transaction pattern, v7.2: write-only, v0.4.0: percentage-based voltage 20-100% of 403.2V max]
...
#define VCU_I2C_DIAGNOSTIC_CONTROL     (0x14)  ///< Diagnostic control overrides (WRITE-ONLY: TX1: 9 bytes total [command + 8 data: flags[2], waterPumpDuty, reserved[5]]; TX2: None - I2C ACK confirms) [v8.0: two-transaction pattern, v7.2: write-only, v7.0: advanced diagnostic testing]
